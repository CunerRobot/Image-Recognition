...
close all
%% Get training images
pipe_ds = imageDatastore('Connectionimage','IncludeSubfolders',true,'LabelSource','foldernames');%创建带标签的数据库
%auds=augmentedImageDatastore([227,227],pipe_ds);%创建增强图片数据库，图像尺寸为227x227
[trainImgs,ValidationImgs] = splitEachLabel(pipe_ds,0.9,'randomize');%数据库分割
...
options = trainingOptions('sgdm',...%使用SGDM优化器
    ...
    'Plots','training-progress');%显示训练过程
                                 %最小批量为16，显示训练过程
%Perform training
[ConnectionNet,info] = trainNetwork(train_auds,layers, options);%用训练集、修改后的训练网络，训练好的网络为ConnectionNet；
%Use trained network to classify test images
save 'ConnectionNet.mat' ConnectionNet%将训练好的网络存储为ConnectionNet，以备调用
%% 真实管道图片测试
testSet = imageDatastore('testimage');
testauds=augmentedImageDatastore([227,227],testSet);
preds=classify(ConnectionNet,testauds);
size=numel(preds);
%显示
